...
public class CorpsePanel extends JPanel implements ActionListener, ComponentListener, KeyListener, DocumentListener {
    ...
    private final String[] organs = { "antenna", "arm", "beak", "bladder", "brain", "ear", "eye", "foot", "gill",
            "heart", "horn", "kidney", "leg", "liver", "lung", "nose", "paw", "snout", "spleen", "stomach", "tail",
            "tendril", "wing" };
    ...
    private CorpseCheckBox tsaraksoul = new CorpseCheckBox("tzarakk chaosfeed", false, "tzarakk chaosfeed corpse", this,
            font);
    private CorpseCheckBox ripSoulToKatana = new CorpseCheckBox("shitKatana rip soul", false, "rip soul from corpse",
            this, font);
    ...
    private CorpseCheckBox eatCorpse = new CorpseCheckBox("get and eat corpse", false,
            "get corpse" + getDelim() + "eat corpse", this, font);
    private CorpseCheckBox donate = new CorpseCheckBox("donate noeq and drop rest", false,
            "get all from corpse" + getDelim() + "donate noeq" + getDelim() + "drop noeq", this, font);
    private CorpseCheckBox lootCorpse = new CorpseCheckBox("get loot from corpse", false,
            "get " + getLootString() + " from corpse", this, font);
    private CorpseCheckBox lootGround = new CorpseCheckBox("get loot from ground", false, "get " + getLootString(),
            this, font);
    private CorpseCheckBox barbarianBurn = new CorpseCheckBox("barbarian burn corpse", false, "barbburn", this, font);
    private CorpseCheckBox feedCorpseTo = new CorpseCheckBox("feed corpse to mount", false,
            "get corpse" + getDelim() + "feed corpse to " + getMountName(), this, font);
    private CorpseCheckBox beheading = new CorpseCheckBox("kharim behead corpse", false,
            "use beheading of departed at corpse", this, font);
    ...
    private CorpseCheckBox vampireWake = new CorpseCheckBox("vampire wake corpse", false, "vampire wake corpse", this,
            font);
    private CorpseCheckBox skeletonWake = new CorpseCheckBox("skeleton wake corpse", false, "skeleton wake corpse",
            this, font);
    private CorpseCheckBox zombieWake = new CorpseCheckBox("zombie wake corpse", false, "zombie wake corpse", this,
            font);
    private CorpseCheckBox aelenaOrgan = new CorpseCheckBox("aelena extract organ", false,
            "familiar harvest antenna antenna", this, font);
    private CorpseCheckBox aelenaFam = new CorpseCheckBox("aelena fam consume corpse", false, "familiar consume corpse",
            this, font);
    private CorpseCheckBox dissect = new CorpseCheckBox("dissection", false, "use dissection at corpse try ", this,
            font);
    private CorpseCheckBox tin = new CorpseCheckBox("tin corpse", false, "tin corpse", this, font);
    private CorpseCheckBox extractEther = new CorpseCheckBox("extract ether", false, "use extract ether at corpse ",
            this, font);
    ...
    private String getLootString() {
        ...
        /**
         * (00:03) Torc tells you ''loottinoids' is a command-alias to 'grep -q -v
         * "There is no" ga mithril,all batium,all anipium,all platinum,all gold,all
         * gem,all box,all
         * chest,all safe,all scroll,all emerald disc,all rune,all compass,all shard,all
         * key from all corpse;grep -q -v "There is no" ga mithril,all batium,all
         * anipium,all
         * platinum,all gold,all chest,all box,all safe,all gem,all scroll,all emerald
         * disc,all rune,all compass,all shard,all key'.'
         * 
         */
    }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (source == kharimSoulCorpse) {
            ...
            turnOff(tin, lichdrain, kharimsoul, tsaraksoul, ripSoulToKatana, eatCorpse, barbarianBurn, feedCorpseTo,
                    beheading, desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam,
                    aelenaOrgan, dig, dissect, extractEther);
        } else if (source == eatCorpse) {
            turnOff(tin, arkemile, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam,
                    aelenaOrgan, extractEther);
        } else if (source == barbarianBurn) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, feedCorpseTo, beheading, desecrateGround,
                    burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan, extractEther);
        } else if (source == feedCorpseTo) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, beheading, desecrateGround,
                    burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan, extractEther);
        } else if (source == beheading) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam,
                    aelenaOrgan, extractEther);
        } else if (source == desecrateGround) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    burialCere, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan, extractEther);
        } else if (source == burialCere) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, lichWake, skeletonWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan,
                    extractEther);
        } else if (source == lichWake) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, skeletonWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan,
                    extractEther);
        } else if (source == vampireWake) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, zombieWake, aelenaFam, aelenaOrgan,
                    extractEther);
        } else if (source == skeletonWake) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, vampireWake, zombieWake, aelenaFam, aelenaOrgan,
                    extractEther);
        } else if (source == zombieWake) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam, aelenaOrgan,
                    extractEther);
        } else if (source == aelenaFam) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaOrgan, extractEther);
        } else if (source == aelenaOrgan) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam, extractEther);
        } else if (source == dig) {
            turnOff(tin, arkemile, eatCorpse, dissect, aelenaOrgan, ripSoulToKatana, barbarianBurn, feedCorpseTo,
                    beheading, desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam,
                    extractEther);
        } else if (source == dissect) {
            turnOff(tin, arkemile, eatCorpse, dig, aelenaOrgan, ripSoulToKatana, barbarianBurn, feedCorpseTo, beheading,
                    desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam, extractEther);
        } else if (source == tin) {
            turnOff(dissect, arkemile, eatCorpse, dig, aelenaOrgan, ripSoulToKatana, barbarianBurn, feedCorpseTo,
                    beheading, desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam,
                    extractEther);
        } else if (source == extractEther) {
            turnOff(tin, arkemile, eatCorpse, dig, dissect, aelenaOrgan, ripSoulToKatana, barbarianBurn, feedCorpseTo,
                    beheading, desecrateGround, burialCere, lichWake, skeletonWake, vampireWake, aelenaFam);
    ...
    private void redoLayout() {
        ...
        soulPanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Souls", TitledBorder.DEFAULT_JUSTIFICATION,
                TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        soulPanel.setBackground(Color.BLACK);
        listPanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Items to loot",
                TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        listPanel.setBackground(Color.BLACK);
        controlPanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Controls",
                TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        controlPanel.setBackground(Color.BLACK);
        wakePanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Wake up corpse!",
                TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        wakePanel.setBackground(Color.BLACK);
        lootPanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Looting", TitledBorder.DEFAULT_JUSTIFICATION,
                TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        lootPanel.setBackground(Color.BLACK);
        corpsePanel.setBorder(BorderFactory.createTitledBorder(whiteline, "Carcasses",
                TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, font, TEXT_COLOR));
        ...
        corpsePanel.setBounds(BORDERLINE * 2, soulPanel.getHeight() + (BORDERLINE * 4),
                (CB_WIDTH * 2) + (2 * TOP_BORDER), (CB_HEIGHT * 7));
        ...
        wakePanel.setBounds(BORDERLINE * 2, corpsePanel.getHeight() + soulPanel.getHeight() + (BORDERLINE * 6),
                (CB_WIDTH * 2) + (2 * TOP_BORDER), (CB_HEIGHT * 5));
        ...
        lootPanel.setBounds(BORDERLINE * 2,
                wakePanel.getHeight() + corpsePanel.getHeight() + soulPanel.getHeight() + (BORDERLINE * 8),
                (CB_WIDTH * 2) + (2 * TOP_BORDER), (CB_HEIGHT * 4));
        ...
        controlPanel.setBounds((BORDERLINE * 4) + lootPanel.getWidth(), BORDERLINE * 2,
                (LABEL_WIDTH * 2) + (2 * TOP_BORDER), (CB_HEIGHT * 9));
        ...
        listPanel.setBounds((BORDERLINE * 4) + lootPanel.getWidth(), controlPanel.getHeight() + (BORDERLINE * 4),
                (LABEL_WIDTH * 2) + (2 * TOP_BORDER), (CB_HEIGHT * 12) + 3);
        listPanel.setLayout(null);
        listPane.setBounds(BORDERLINE * 2, TOP_BORDER, listPanel.getWidth() - (4 * BORDERLINE),
                listPanel.getHeight() - ((2 * TOP_BORDER) + CB_HEIGHT));
